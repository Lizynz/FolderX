...
@interface ColourOptionsComentController ()
- (void)coloursView;
@end
...
@implementation ColourOptionsComentController
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)loadView {
    ...
    UIColor *color = [unarchiver1 decodeObjectForKey:NSKeyedArchiveRootObjectKey];
...
@implementation ColourOptionsComentController(Privates)

- (void)save {
    NSData *colorData1 = [NSKeyedArchiver archivedDataWithRootObject:self.selectedColor requiringSecureCoding:nil error:nil];
    NSUserDefaults *defaults = [[NSUserDefaults alloc] initWithSuiteName:saveSettings];
    [defaults setObject:colorData1 forKey:@"kTitleColor"];
...
- (void)center {
    ...
    NSString *domain = saveSettings;
    NSUserDefaults *defaults = [[NSUserDefaults alloc] initWithSuiteName:domain];
    [defaults removeObjectForKey:@"kTitleColor"];
    [defaults synchronize];
}
